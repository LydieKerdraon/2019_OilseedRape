...
load("~/Emphasis_11.10.18/Emphasis/Manip/LM_nonLM/analyse_stats/Fung_bact/NetworkData.RData")
...
x<-data.frame()
COND<-list('2017-07','2017-10','2017-12','2018-02')
for (i in COND){
  
  ## Condition
  
  Bact<-subset_samples(phyloBactAll, date == i)
  Fung<-subset_samples(phyloFungAll, date == i)
  ...
  set.seed(1)
  plot(delete.vertices((adj2igraph(getRefit(spiec), vertex.attr=list(name=dtype,color=my_color, label.cex=0.001,label.color=my_color,shape=dtypeShape,size=7),
                                   edge.attr=list(width=13,color=(ifelse(E(ig)$weight > 0.0000, 'green', 'red'))))),names(which(nodedegree == "TRUE"))),)+title(i,cex.main = 4)     
  ...
  assign(paste("PlotBetweeness_",i, sep = ""),ggplot(BetTax, aes_string(x="deg", y="bet"))+geom_point(aes_string(shape="Kingdom", color="Class"),size=2)+theme_bw()+scale_colour_manual(values=my_color)+scale_shape_manual(values=c(16, 15))+ggtitle(i)+ theme(legend.position="none")+xlim(0,10)+ylim(0,2600)
  )
...
write.csv(x, paste(path,"networks_data.csv"))

library(cowplot)
plot_grid(`PlotBetweeness_2017-07`,`PlotBetweeness_2017-10`,`PlotBetweeness_2017-12`,
          `PlotBetweeness_2018-02`)
